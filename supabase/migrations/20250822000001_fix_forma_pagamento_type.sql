/*
# [FIX FORMA PAGAMENTO TYPE]

Esta migração corrige o problema de tipo na função registrar_recebimento_parcial
para aceitar forma_pagamento como TEXT em vez de um tipo específico.
*/

-- Criar ou substituir a função registrar_recebimento_parcial com correção de tipo
...
-- Criar também a função registrar_recebimento para pagamentos completos
...
COMMENT ON FUNCTION registrar_recebimento_parcial(uuid, numeric, text, date, text, uuid) IS 'Registra recebimento parcial de fatura com opções para valor remanescente';
COMMENT ON FUNCTION registrar_recebimento(uuid, numeric, text, date, uuid) IS 'Registra recebimento completo de fatura';